pipeline {
  ...
  stages {
    stage('Limpiar') {
      steps {
        sh '''docker stop portal-empleado-green && docker rm portal-empleado-green
...
'''
        ...
        stage('Tests E2E') {
          steps {
            sh '''docker run -d -p 4444:4444 --net mynet -v /dev/shm:/dev/shm --name firefox-container selenium/standalone-firefox:3.12.0-americium
...
docker run --net mynet -p 6666:6666 -v /$(pwd)/e2eTests:/e2eTests -v /$(pwd)/package.json:/package.json -v /$(pwd)/src:/src --rm portal-empleado-e2e-test'''
    ...
    stage('Desplegar') {
      steps {
        sh '''docker stop portal-empleado-test && docker rm portal-empleado-test
docker build --tag portal-empleado-green:$BUILD_NUMBER .
